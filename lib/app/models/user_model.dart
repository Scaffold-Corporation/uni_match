...
class UserModel{
  ...
  /// Get user object => [Usuario]
  Future<Usuario> getUserObject(String userId) async {
    /// Get Updated user info
    final DocumentSnapshot userDoc = await UserModel().getUser(userId);

    /// return user object
    return Usuario.fromDocument(userDoc.data()! as Map);
  }
  ...
  Future<void> authUserAccount({
    ...
    if (getFirebaseUser != null) {
      ...
      debugPrint("firebaseUser não conectado");
      signInScreen!();
    }
  }
  ...
  List<String> getUserProfileImages(Usuario user) {
    // Get profile photo
